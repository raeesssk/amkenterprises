// import admin
angular.module('purchase').controller('purchaseCtrl', function ($rootScope, $http, $scope, $location, $routeParams, $route, $filter) {
 ...
 $scope.apiURL = $rootScope.baseURL+'/purchase/purchase/total';
    ...
    $scope.deleteSale = function (prm_id) {
      $scope.prm_id=prm_id;
    }  
    ...
    $scope.reopenPurchase = function (sm_id) {
    ...
    $scope.reopenConfirm = function () {
       ...
       $http({
        method: 'POST',
        url: $rootScope.baseURL+'/purchase/reopen/'+$scope.sm_id.prm_id,
        data: $scope.sm_id,
        headers: {'Content-Type': 'application/json',
                  'Authorization' :'Bearer '+localStorage.getItem("amkenterprises_admin_access_token")}
      })
  ...
  $scope.viewQuatationDetails = function (index) {
    ...
    $scope.printDetails = function(){
      ...
      if($scope.purchases.prm_status == 1){
